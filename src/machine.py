...
class boltzmann():
    ...
    def create_network(self, distances, node_map=string.ascii_uppercase):
        ...
        map_weights = np.vectorize(
            lambda w: -100 * math.exp(-w) if w != 0 else 0.0)
        ...
        for city in range(n_count):
            for epoch in range(t_count):
                ...
                self.network[city, epoch].weights = np.matrix(
                    np.zeros(n_count * t_count))
                ...
                self.network[city, epoch].weights[:,
                                                  (epoch + 1) % t_count] = modified_distances[city].T
                self.network[city, epoch].weights[:,
                                                  (epoch - 1) % t_count] = modified_distances[city].T

                # avoid visiting the same city twice in a tour
                self.network[city,
                             epoch].weights[city] = self.hamiltonian_error_charge

                # avoid visiting more than one city within an epoch
                self.network[city, epoch].weights[:,
                                                  epoch] = self.hamiltonian_error_charge
                ...
                if epoch == 0:
                    self.network[city, epoch].weights[:, -
                                                      1] = self.hamiltonian_error_charge
                    self.network[city, epoch].weights[city, -1] = -1
                elif epoch == t_count - 1:
                    self.network[city, epoch].weights[:,
                                                      0] = self.hamiltonian_error_charge
